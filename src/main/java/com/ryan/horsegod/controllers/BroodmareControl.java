...
public class BroodmareControl {
	...
	public BroodmareControl(BroodmareService broodServ, StudService studServ, DamService damServ, SireService sireServ, FoalService foalServ, MareService mareServ, StallionService stallServ, FillyService fillyServ, ColtService coltServ, UserService userServ) {
	...
	public String allBroods(Principal principal, Model model) {
		...
		List<Broodmare> brood = broodServ.allBroods();
	...
	public String confirmPairing(@PathVariable(value="studId") UUID studId, @PathVariable(value="broodId") UUID broodId, @ModelAttribute("stud") Stud stud, @ModelAttribute("brood") Broodmare brood, BindingResult result, Principal principal, Model model) {
		String username = principal.getName();
